...
const testResult = (array) => {
    let a = [+[array.reduce((acc,value) => acc + value, 0) / array.length][0].toFixed(3),
        {
            h: array.filter(el => el >=9 ).length,
            a: array.filter(el => el >= 7 && el <= 8).length,
            l: array.filter(el => el < 7).length
        }]
    a[1]["h"] === array.length && a.push("They did well")
    return  a
}